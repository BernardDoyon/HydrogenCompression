function [t,rho, Tair, P, Trock,r,Nc] = fcnSolve_dPdt_kRvariable(NbCycles,para,SolverOptions,flagRealGas)
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for n = 1:NbCycles
         if (para.underground)
            if (n ==1) %temperature of cavity is constant everywhere
                % evaluate the maximal value of F0*kR to estimate Rp (Eq
                % A.7 of Kushnir)
                Rchange = para.ThermalPropertiesFunction(1,para.T0); % get the values of R where material is changing
                NbKr = length(Rchange);
                [kR,F0] = fcnKrFo(para,Rchange-1e-6,ones(1,NbKr)*para.T0*para.TRw0);
            else %temperature of cavity is different for every r values
                [kR,F0] = fcnKrFo(para,r(n,:)*para_Rint,Yinitial(1,1:N+1)*para.T0);
            end
            maxFokR = max(F0.*kR);
        ...
        if (NtimeStep) % We read the solution at NtimeStep + 1 (t=0)
            for i = 1:NtimeStep
                ...
                if (flagRealGas) %real gaz
                    sol = solver(@(t,Y) odefcn_dPdt_Real(t,Y,r,eta_p,eta_pp,para,mdot),tspan,Yspan(i,:),options);
                else % simplified gaz
                    sol = solver(@(t,Y) odefcn_dPdt_Simple(t,Y,r,eta_p,eta_pp,para,mdot),tspan,Yspan(i,:),options);
                end
            ...
            if (flagRealGas) 
                mdot = 0; % 
                [tspan,Yspan] = solver(@(t,Y) odefcn_dPdt_Real(t,Y,r,eta_p,eta_pp,para,mdot),tspan,Yinitial,options);
            else
                [tspan,Yspan] = solver(@(t,Y) odefcn_dPdt_Simple(t,Y,r,eta_p,eta_pp,para,mdot),tspan,Yinitial,options);
            end
        end
...
function dYdt = odefcn_dPdt_Simple(t,Y,r,eta_p,eta_pp,para,mdot)
    ...
    % values of kR and F0 for temperature T and position r (in meters)
    
    [kR,F0] = fcnKrFo(para,r*para.Rw_int,Y(1:N+1)*para.T0);
    ...
    if (para.Nusselt) % try to estimate heat exchange coefficient from Nusselt parameter
        ...
        Bi_int = hc*para.Rw_int/kR(1);
        qp = hc*para.qpSimple;
    else
        Bi_int = para.hc_int*para.Rw_int/kR(1);
%        Bi_int = para.Bi_int;
        qp = para.qpSimple;
        %{
...
function dYdt = odefcn_dPdt_Real(t,Y,r,eta_p,eta_pp,para,mdot)
    % Charge: Fie >0 
    % Y(N+3) -> rho*
    % Y(N+2) -> T*
    % Y(N+4) -> P*
    ...
    elseif (para.Gie(t)>0)
        ...
        dYdt(N+2) = para.pr/cps*(Qloss*(alpha/beta*(cps-cv)*para.Gie(t)) + qp/Y(N+3)*(Y(1)-Y(N+2)));  
        dYdt(N+3) = Y(N+3)*(alpha*para.Gie(t)*para.pr-beta*dYdt(N+2));
        dYdt(N+4) = para.Gie(t)*para.pr;
    else
        cp = para.Cp(Y(N+2)*para.T0,Y(N+4)*para.P0);
...
function [kR, F0] = fcnKrFo(para,r,T)

     NbValues = length(r);
     kR = zeros(1,NbValues);
     CpR = zeros(1,NbValues);
     rhoR = zeros(1,NbValues);
    
     for n = 1:NbValues
        [temp, kR(n),CpR(n),rhoR(n)] = para.ThermalPropertiesFunction(r(n),T(n));
     end
     alpha = kR./CpR./rhoR;
     F0 = para.tp/(para.Rw_int)^2.*alpha./kR; 
end
