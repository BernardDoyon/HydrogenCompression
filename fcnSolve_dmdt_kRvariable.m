...
function [t,rho, Tair, P, Trock,r,Nc] = fcnSolve_dmdt_kRvariable(NbCycles,para,SolverOptions,flagRealGas)
%
        ...
        switch (flagRealGas)
            case 0 % Simple or ideal gas model 
                if (NtimeStep) % We read the solution at NtimeStep + 1 (t=0)
                    sol = solver(@(t,Y) odefcn_dmdt_Simple(t,Y,r(n,:),eta_p,eta_pp,para),tspan,Yinitial,options);
                    tspan = linspace(0,1,NtimeStep+1);
                    Yspan = (deval(sol,tspan)).';
                else % we take only the time steps choosen by the numerical integrator
                    [tspan,Yspan] = solver(@(t,Y) odefcn_dmdt_Simple(t,Y,r(n,:),eta_p,eta_pp,para),tspan,Yinitial,options);
                end
            case 1 % Real gas model 
                 if (NtimeStep) % We read the solution at NtimeStep + 1 (t=0)
                    sol = solver(@(t,Y) odefcn_dmdt_Real(t,Y,r(n,:),eta_p,eta_pp,para),tspan,Yinitial,options);
                    tspan = linspace(0,1,NtimeStep+1);
                    Yspan = (deval(sol,tspan)).';
                else % we take only the time steps choosen by the numerical integrator
                    [tspan,Yspan] = solver(@(t,Y) odefcn_dmdt_Real(t,Y,r(n,:),eta_p,eta_pp,para),tspan,Yinitial,options);
                end
        end
...
function dYdt = odefcn_dmdt_Real(t,Y,r,eta_p,eta_pp,para)
...
function dYdt = odefcn_dmdt_Simple(t,Y,r,eta_p,eta_pp,para)
